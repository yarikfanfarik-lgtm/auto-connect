...
class BluetoothScanService : Service() {
    ...
    override fun onCreate() {
        ...
        val notification = Notification.Builder(this, channelId)
            ...
            .setContentText("Ищем знакомые Bluetooth-устройства рядом")
    ...
    private fun maybeShow(device: BluetoothDevice) {
        ...
        if (name.isBlank() || !isKnown(name) || !Settings.canDrawOverlays(this)) return
        showPopup(device, name)
    }

    private fun isKnown(name: String): Boolean {
        val n = name.lowercase(Locale.getDefault())
        return listOf("airpods", "buds", "watch", "headphones", "wh-1000xm", "beats", "galaxy watch").any { n.contains(it) }
    }
    ...
    private fun showPopup(device: BluetoothDevice, name: String) {
        ...
        val subtitle = TextView(this).apply { text = "Устройство рядом"; textSize = 15f; setTextColor(Color.DKGRAY); gravity = Gravity.CENTER }
